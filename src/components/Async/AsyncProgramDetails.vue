    ...
    <div v-else class="row my-5 justify-content-center" >
                    <div class="sec-order col-12 col-lg-7 rtl gy-5"> 
                        <div class="d-flex flex-column flex-md-row text-white my-3">
                            <a @click="accept" v-if="userStore.userType == '1' && !edit"  class="btn btn-lg btn-bd-primary mb-3 br-green " >قبول البرنامج </a>
                            ...
                            <a @click="deletee" v-if="userStore.userType == '1'" class="btn btn-lg  mb-3 mx-3 btn-danger " >حذف البرنامج </a>
                        </div>  
...
const userStore= useUserStore()
    ...
    const edit = ref(true)
        ...
        const deletee = async ()=>{
                const { awaitdata,awaiterror } = await useDelete('api/Program/destroy/'+data.value.id)
                if(awaitdata.value)
                {
                    router.push({name:'showprograms'})
                }
        }
